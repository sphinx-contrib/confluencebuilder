...
def main():
    ...
    if target_unit_test_name_pattern:
        target_unit_tests, issued = find_tests(
            unit_tests, target_unit_test_name_pattern
        )
        ...
        print("")
        print("running specific test(s) (total: {})".format(len(target_unit_tests)))
    ...
    # invoke test suite
    runner = unittest.TextTestRunner(buffer=buffered, verbosity=DEFAULT_VERBOSITY)
