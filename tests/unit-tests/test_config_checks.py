...
from tests.lib import EXT_NAME, mock_getpass, mock_input, prepare_conf, prepare_sphinx
...
class TestConfluenceConfigChecks(unittest.TestCase):
    ...
    def test_config_check_client_cert(self):
        ...
        # too many
        self.config["confluence_client_cert"] = (valid_cert, valid_cert, valid_cert)
        with self.assertRaises(ConfluenceConfigurationError):
            self._try_config()
    ...
    def test_config_check_jira_servers(self):
        ...
        self.config["confluence_jira_servers"] = {"server-1": None}
        with self.assertRaises(ConfluenceConfigurationError):
            self._try_config()

        self.config["confluence_jira_servers"] = {"server-1": {}}
        with self.assertRaises(ConfluenceConfigurationError):
            self._try_config()
    ...
    def test_config_check_server_url(self):
        self.config["confluence_server_url"] = "https://example.atlassian.net/wiki/"
        self._try_config()
        ...
        self.config["confluence_server_url"] = "https://intranet-wiki.example.org/"
        self._try_config()
