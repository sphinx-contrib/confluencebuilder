...
class TestConfluenceRstContents(unittest.TestCase):
    ...
    def test_storage_rst_contents_backlinks_entry(self):
        ...
        out_dir = build_sphinx(
            self.dataset, config=self.config, filenames=["contents-backlinks-entry"]
        )
    ...
    def test_storage_rst_contents_backlinks_none(self):
        ...
        out_dir = build_sphinx(
            self.dataset, config=self.config, filenames=["contents-backlinks-none"]
        )
    ...
    def test_storage_rst_contents_backlinks_top(self):
        ...
        out_dir = build_sphinx(
            self.dataset, config=self.config, filenames=["contents-backlinks-top"]
        )
    ...
    def test_storage_rst_contents_caption(self):
        out_dir = build_sphinx(
            self.dataset, config=self.config, filenames=["contents-caption"]
        )

        with parse("contents-caption", out_dir) as data:
            caption = data.find(re.compile("^h[1-6]$"), text="table of contents")
    ...
    def test_storage_rst_contents_default_title(self):
        ...
        out_dir = build_sphinx(self.dataset, config=config, filenames=["contents"])
    ...
    def test_storage_rst_contents_default_top(self):
        out_dir = build_sphinx(self.dataset, config=self.config, filenames=["contents"])
    ...
    def test_storage_rst_contents_local(self):
        out_dir = build_sphinx(
            self.dataset, config=self.config, filenames=["contents-local"]
        )
