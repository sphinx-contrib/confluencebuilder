...
class TestConfluenceRstAdmonitions(unittest.TestCase):
    ...
    def test_storage_rst_admonitions(self):
        ...
        with parse("admonitions", out_dir) as data:
            ...
            self.assertIsNotNone(title_param, "admonition is missing a title paramater")
            self.assertEqual(
                title_param.text,
                "my-title",
            ...
            self.assertIsNotNone(icon_param, "admonition is missing a icon paramater")
            self.assertEqual(
                icon_param.text, "false", "admonition icon value is not disabled"
            )
    ...
    def _verify_storage_tags(self, data, text, expected_type):
        ...
        self.assertIsNotNone(leaf, "unable to find target admonition " + text)
