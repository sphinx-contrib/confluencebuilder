...
class TestConfluenceRstFigure(unittest.TestCase):
    ...
    def test_storage_rst_figure_defaults(self):
        ...
        with parse("figure", out_dir) as data:
            figures = data.find_all(
                "ac:structured-macro", {"ac:name": "info"}, recursive=False
            )
            self.assertEqual(len(figures), 5)

            for figure in figures:
                icon_param = figure.find(
                    "ac:parameter", {"ac:name": "icon"}, recursive=False
                )
            ...
            self.assertEqual(url["ri:value"], "https://www.example.com/image.png")
            ...
            url = image.find("ri:url")
            self.assertTrue(url.has_attr("ri:value"))
            self.assertEqual(url["ri:value"], "https://www.example.org/image.png")
