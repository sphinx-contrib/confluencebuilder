...
from sphinxcontrib.confluencebuilder.config.notifications import deprecated, warnings
...
def validate_configuration(builder):
    ...
    validator.conf("confluence_add_secnumbers").bool()

    # ##################################################################

    # confluence_additional_mime_types
    validator.conf("confluence_additional_mime_types").strings(no_space=True)

    # ##################################################################

    # confluence_adv_aggressive_search
    validator.conf("confluence_adv_aggressive_search").bool()

    # ##################################################################

    # confluence_adv_hierarchy_child_macro
    validator.conf("confluence_adv_hierarchy_child_macro").bool()

    # ##################################################################

    # confluence_adv_permit_raw_html
    validator.conf("confluence_adv_permit_raw_html").bool()

    # ##################################################################

    # confluence_adv_trace_data
    validator.conf("confluence_adv_trace_data").bool()

    # ##################################################################

    # confluence_adv_writer_no_section_cap
    validator.conf("confluence_adv_writer_no_section_cap").bool()

    # ##################################################################

    # confluence_ask_password
    validator.conf("confluence_ask_password").bool()

    # ##################################################################

    # confluence_ask_user
    validator.conf("confluence_ask_user").bool()

    # ##################################################################

    # confluence_asset_force_standalone
    validator.conf("confluence_asset_force_standalone").bool()

    # ##################################################################

    validator.conf("confluence_ca_cert").file()
...
"""
    ...
    validator.conf("confluence_client_cert_pass").string()

    # ##################################################################

    # confluence_default_alignment
    try:
        validator.conf("confluence_default_alignment").matching(
            "left", "center", "right"
        )
...
"""
    ...
    validator.conf("confluence_disable_autogen_title").bool()

    # ##################################################################

    # confluence_disable_notifications
    validator.conf("confluence_disable_notifications").bool()

    # ##################################################################

    # confluence_disable_ssl_validation
    validator.conf("confluence_disable_ssl_validation").bool()

    # ##################################################################

    # confluence_file_suffix
    validator.conf("confluence_file_suffix").string()

    # ##################################################################

    # confluence_file_transform
    validator.conf("confluence_file_transform").callable_()

    # ##################################################################

    # confluence_footer_file
    try:
        validator.conf("confluence_footer_file").string().file()
    except ConfluenceConfigurationError as e:
        raise ConfluenceConfigurationError(
            """%s
...
"""
    ...
    try:
        validator.conf("confluence_global_labels").strings(no_space=True)
    except ConfluenceConfigurationError as e:
        raise ConfluenceConfigurationError(
            """%s
...
"""
    ...
    try:
        validator.conf("confluence_header_file").string().file()
    except ConfluenceConfigurationError as e:
        raise ConfluenceConfigurationError(
            """%s
...
"""
    ...
    validator.conf("confluence_ignore_titlefix_on_index").bool()
    ...
    if config.confluence_jira_servers is not None:
        ...
        if not isinstance(jira_servers, dict):
            ...
            for name, info in jira_servers.items():
                ...
                if not (
                    isinstance(jira_id, basestring)
                    and isinstance(jira_name, basestring)
                ):
                    issue = True
                    break
...
"""
    ...
    validator.conf("confluence_lang_transform").callable_()

    # ##################################################################

    # confluence_link_suffix
    validator.conf("confluence_link_suffix").string()

    # ##################################################################

    # confluence_link_transform
    validator.conf("confluence_link_transform").callable_()

    # ##################################################################

    # confluence_root_homepage
    validator.conf("confluence_root_homepage").bool()

    # ##################################################################

    # confluence_max_doc_depth
    try:
        validator.conf("confluence_max_doc_depth").int_()
    except ConfluenceConfigurationError as e:
        raise ConfluenceConfigurationError(
            """%s
...
"""
    ...
    validator.conf("confluence_parent_page_id_check").int_(positive=True)

    # ##################################################################

    # confluence_page_hierarchy
    validator.conf("confluence_page_hierarchy").bool()

    # ##################################################################

    # confluence_prev_next_buttons_location
    try:
        validator.conf("confluence_prev_next_buttons_location").matching(
            "bottom", "both", "top"
        )
    except ConfluenceConfigurationError as e:
        raise ConfluenceConfigurationError(
...
"""
    ...
    # confluence_proxy
    validator.conf("confluence_proxy").string()
    ...
    for option in publish_list_options:
        ...
        try:
            validator.conf(option, conf_translate).string_or_strings()
...
"""
    ...
    validator.conf("confluence_publish_dryrun").bool()

    # ##################################################################

    # confluence_publish_headers
    validator.conf("confluence_publish_headers").dict_str_str()

    # ##################################################################

    # confluence_publish_onlynew
    validator.conf("confluence_publish_onlynew").bool()

    # ##################################################################

    # confluence_publish_postfix
    validator.conf("confluence_publish_postfix").string()

    # ##################################################################

    # confluence_publish_prefix
    validator.conf("confluence_publish_prefix").string()

    # ##################################################################

    validator.conf("confluence_publish_root").int_(positive=True)

    # ##################################################################

    # confluence_purge
    validator.conf("confluence_purge").bool()

    # ##################################################################

    # confluence_purge_from_root
    validator.conf("confluence_purge_from_root").bool()

    # ##################################################################

    # confluence_remove_title
    validator.conf("confluence_remove_title").bool()

    # ##################################################################

    # confluence_secnumber_suffix
    validator.conf("confluence_secnumber_suffix").string(permit_empty=True)
...
"""
    ...
    validator.conf("confluence_server_cookies").dict_str_str()

    # ##################################################################

    # confluence_server_pass
    validator.conf("confluence_server_pass").string()

    # ##################################################################

    # confluence_server_url
    validator.conf("confluence_server_url").string()

    # ##################################################################

    # confluence_server_user
    validator.conf("confluence_server_user").string()

    # ##################################################################

    # confluence_space_name
    validator.conf("confluence_space_name").string()

    # ##################################################################

    # confluence_title_overrides
    validator.conf("confluence_title_overrides").dict_str_str()

    # ##################################################################

    # confluence_timeout
    try:
        validator.conf("confluence_timeout").int_()
    except ConfluenceConfigurationError as e:
        raise ConfluenceConfigurationError(
            """%s
...
"""
    ...
    # confluence_watch
    validator.conf("confluence_watch").bool()
...
"""
        ...
        if (
            config.confluence_ask_password
            and not config.confluence_server_user
